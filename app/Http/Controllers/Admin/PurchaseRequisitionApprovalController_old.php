...
class PurchaseRequisitionApprovalController extends Controller
{
    ...
    public function edit($id,Request $request){
        ...
        if($request->ajax()){
            ...
            if(isset($request->payment_terms)){
                ...
                $payment_terms_all = PurchaseRequisition::select('payment_terms')->where('payment_terms','like','%'.$request->payment_terms.'%')->get();
                $payment_terms = $payment_terms_all->unique();
                $append_value = '';
    ...
    public function export(){
        $purchase_requisition_approval_data = PurchaseRequisitionDetails::select('purchase_requisition_detail.*','company_masters.company_name','supplier_masters.supplier_name','purchase_requisition.purchase_approval_date','purchase_requisition.purchase_approval_status','purchase_requisition.total_price','purchase_requisition.dollar_total_price','purchase_requisition.po_no')
                                    ->leftjoin('purchase_requisition','purchase_requisition_detail.purchase_requisition_id','=','purchase_requisition.id')
        ...
        Excel::create($po_approval_csv_name, function($excel) use($purchase_requisition_approval_data){
            $excel->sheet('PO Approval Report', function($sheet) use($purchase_requisition_approval_data){
                ...
                $sheet->row(1,['Id','Company Name','Creation Date','Approval Date','Supplier Name','Total Price in INR','Total Price in USD','Purchase Approval Status','Po No']);
    ...
    public function reorder(){
        ...
        return view('admin.purchase_requisition_approval.reorder');
    ...
    public function restoreOrders(Request $request){
        ...
        PurchaseRequisition::where('po_no',$restoreId)->restore();
        return $restoreId;
