...
<nav class="navbar navbar-static-top">
    <div class="title">
        <h4><i class="fa fa-list"></i> Re Order Purchase Requisition</h4>
    </div>
    ...
    $(function(){
        ...
        $('#purchase_requisition_table').DataTable({
            ...
            {   mData:"company_name",sWidth:"15%",bSortable : true,
                mRender : function(v,t,o){  
                    var is_check = "<?= App\Helpers\DesignationPermissionCheck::isPermitted('product.edit') && App\Helpers\DesignationPermissionCheck::isPermitted('purchase-requisition.edit')?>";
                    if(is_check != 0){
                        var edit_path = "{{ route('purchase.get',':id') }}";
                        edit_path = edit_path.replace(':id',o['id']);

                        var act_html  = '<a title="Edit '+v+'" href="'+edit_path+'">'+ v +'</a>'
                    }
                    else{
                        var act_html  = '<a title="Edit '+v+'">'+ v +'</a>'
                    }
