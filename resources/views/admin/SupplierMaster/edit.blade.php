...
@stop
@section('start_form')
<?=Form::model($supplierupdate_data,['route' => ['manufacturer.update',$id],'method' => 'patch', 'class' => 'm-0 form-horizontal'])?>
@stop
    ...
    <div class="col-md-12 mb-30">
        <div class="card">
            <div class="card-title-w-btn">
                <h4 class="title">Edit Manufacturer Master</h4>
            </div><hr>
            <div class="row">
                <div class="col-md-12">
                    <div class="row">
...
$(document).ready(function(){
        ...
        if(country_val.val() != null)
        {
            ...
            var supplier_data_info = {!! json_encode($supplier_data_info) !!};
            
            $(supplier_data_info).each(function(supplier_data_key,supplier_data_value){
                $(resp).each(function(k,v){
                    // var t = "<option selected value='"+v.id+"'>"+ v.text+"</option>";
                    if(supplier_data_value.state_id == v.id){
                        $('#state_id'+supplier_data_key).append("<option selected value='"+v.id+"'>"+ v.text+"</option>");

                        respcity = getState(v.id);
                        // console.log(supplier_data_value.city_id);
                        $(respcity).each(function(kc,vc){
                            // console.log(vc.id);
                            if(supplier_data_value.city_id == vc.id){
                                $('#city_id'+supplier_data_key).append("<option selected value='"+vc.id+"'>"+ vc.text+"</option>");
                            }else{
                                // console.log('hi');
                                $('#city_id'+supplier_data_key ).append($('<option>', {value: vc.id, text: vc.text}));
                            }
                        });

                    }else{
                        $('#state_id'+supplier_data_key).append($('<option>', {value: v.id, text: v.text}));
                    }
                });
            });
            var city_data = $('.city_id').html();
            var state_data = $('.state_id').html();
            //console.log(supplier_data_info);
        }
    });
    ...
    old_data = <?= json_encode(old('shipping.shipping',$supplier_data_info)) ?>;
